...
export default function ApartmentReview(props) {
  ...
  const fetchReviews = async () => {
    try {
      const response = await fetch(`http://192.168.1.111:5000/api/Review/GetReviewsForApartment/${apt}`);
  ...
  const deleteReview = async (reviewId) => {
    Alert.alert("מחיקת ביקורת", "אתה בטוח שאתה רוצה למחוק את הביקורת?", [
      ...
      },
      {
        ...
        onPress: async () => {
          try {
            const response = await fetch(`http://192.168.1.111:5000/api/Review/DeleteReview/${reviewId}`, {
              method: "DELETE",
              headers: {
                Accept: "text/plain",
              },
